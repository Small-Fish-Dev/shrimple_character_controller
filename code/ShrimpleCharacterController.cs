...
public class ShrimpleCharacterController : Component
{
	...
	[Property]
	[Group( "PushEnabled" )]
	public Dictionary<string, float> PushTagsWeight { get; set; } = new Dictionary<string, float>() { { "player", 1f } };
	...
	private SceneTraceResult BuildPushTrace( BBox bounds, Vector3 from, Vector3 to )
	{
		return Game.SceneTrace.Box( bounds, from, to )
			.IgnoreGameObjectHierarchy( GameObject )
			.WithAnyTags( PushTagsWeight.Keys.ToArray() ) // Check for only the push tags
			.Run();
	}
